...
def _parse_objects(data: bytes) -> dict[int, bytes]:
    ...
    for _, body in list(objects.items()):
        ...
        header_numbers = decoded[:first].decode("latin1", "ignore").strip().split()
        numbers = list(map(int, header_numbers))
        ...
        if match:
            ...
            for _ in range(count):
                if index >= len(lines):
                    break
                line = lines[index]
                triplet = re.match(r"<([0-9A-Fa-f]+)>\s*<([0-9A-Fa-f]+)>\s*<([0-9A-Fa-f]+)>", line)
                if triplet:
...
def _decode_content_stream(
    content: bytes | None,
    font_refs: dict[str, int],
    font_maps: dict[int, dict[bytes, str]],
) -> list[str]:
    if not content:
        return []
    ...
    for match in _TEXT_PATTERN.finditer(content):
        font_name, text_array, hex_text, literal_text, td_y_1, td_y_2, _td_x, td_y_3 = match.groups()
...
def _collapse_spaced_text(text: str) -> str:
    # Some PDFs emit every Chinese character and every digit as a standalone token.
    # Collapse those artificial spaces before downstream parsing.
    unit_chars = "\u5e74\u6708\u65e5\u5b63\u5ea6\u671f\u6b21\u5143\u80a1\u4eba\u5bb6\u9879\u9875\u53f7"
    patterns = [
        r"(?<=[\u4e00-\u9fff])\s+(?=[\u4e00-\u9fff])",
        r"(?<=[A-Za-z])\s+(?=[A-Za-z])",
        r"(?<=\d)\s+(?=\d)",
...
def _extract_text_from_raw_pdf(path: Path) -> str:
    ...
    for _, body in sorted(objects.items()):
        ...
        content_refs = [int(item) for item in re.findall(rb"(\d+)\s+0\s+R", contents_match.group(1))] if contents_match else []
        if not content_refs:
        ...
        for ref in content_refs:
            stream = _stream_bytes(objects.get(ref, b""))
            parts.extend(_decode_content_stream(stream, font_refs, font_maps))
...
def extract_text(path: Path) -> str:
    ...
    if text:
        return text
    return _extract_text_from_raw_pdf(path)
...
def infer_pdf_metadata(path: Path, text: str = "") -> dict[str, Any]:
    ...
    if not metadata["stock_code"]:
        match = re.search("\u8bc1\u5238\u4ee3\u7801[:\uff1a]?([0-9]{6})", head)
        if match:
            metadata["stock_code"] = match.group(1)
    if not metadata["stock_name"]:
        match = re.search("\u8bc1\u5238\u7b80\u79f0[:\uff1a]?([A-Za-z0-9\u4e00-\u9fff]+)", head)
        if match:
    ...
    if year:
        if any(keyword in title_probe for keyword in ("\u7b2c\u4e00\u5b63\u5ea6\u62a5\u544a", "\u4e00\u5b63\u5ea6\u62a5\u544a")):
            metadata["report_period"] = f"{year}Q1"
        elif any(keyword in title_probe for keyword in ("\u7b2c\u4e8c\u5b63\u5ea6\u62a5\u544a", "\u4e8c\u5b63\u5ea6\u62a5\u544a")):
            metadata["report_period"] = f"{year}Q2"
        elif any(keyword in title_probe for keyword in ("\u7b2c\u4e09\u5b63\u5ea6\u62a5\u544a", "\u4e09\u5b63\u5ea6\u62a5\u544a")):
            metadata["report_period"] = f"{year}Q3"
        elif any(keyword in title_probe for keyword in ("\u534a\u5e74\u5ea6\u62a5\u544a", "\u534a\u5e74\u5ea6\u62a5\u544a\u6458\u8981", "\u534a\u5e74")):
